...
from environments.environments_minigrid import (
    get_simplecross_env,
    make_env_four_rooms,
    make_env_simple_crossing,
    make_env_unlock,
    get_unlock_env,
    make_env_multiroom,
)
from environments.environments_minihack import get_minihack_env, make_env_minihack
...
def get_single_environment(args, seed, problem=None, is_test=False, options=None):
    ...
    if args.env_id == "MiniGrid-SimpleCrossingS9N1-v0":
        ...
        env = get_unlock_env(view_size=args.game_width, seed=seed, options=options)
    elif args.env_id == "MiniHack-Corridor-R2-v0":
        env = get_minihack_env(env_id="MiniHack-Corridor-R2-v0", view_size=args.game_width, seed=seed)
...
def get_single_environment_builder(args, seed, problem=None, options=None, is_test=False):
    ...
    if args.env_id == "MiniGrid-SimpleCrossingS9N1-v0":
        ...
        if not problem:
            problem = COMBO_PROBLEM_NAMES[seed]
        reward_per_step = 0 if is_test else -1
        reward_goal = 10 if is_test else 1
        ...
        env_fn = make_env_multiroom(view_size=args.game_width, seed=seed, options=options)
    elif args.env_id == "MiniHack-Corridor-R2-v0":
        env_fn = make_env_minihack(env_id="MiniHack-Corridor-R2-v0", seed=seed, view_size=args.game_width)
    else:
        raise NotImplementedError
    return env_fn
