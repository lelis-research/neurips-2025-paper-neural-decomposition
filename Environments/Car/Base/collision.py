...
def get_all_vertices(x, y, ang, w, h):
    # Compute the four corners of the car
    res = []
    db = w / 2.0
    da = h / 2.0
    coa = np.cos(ang)
...
def check_collision_point(a_x, a_y, a_r, b_xl, b_xu, b_yl, b_yu, s):
    ...
    if s == 'l':
        ...
        d_max = np.max([d1, d3])
    return -d_max if d_max < 0 else 0
...
def check_collision_box(a_x, a_y, a_ang, b_x, b_y, s, w, h):
    ...
    error += check_collision_point(a_x, a_y, w/2.0 + 0.2,
                                   b_x - w/2.0, b_x + w/2.0,
                                   b_y - h/2.0, b_y + h/2.0, s)
    vertices = get_all_vertices(a_x, a_y, a_ang, w, h)
    
    # check that all vertices of a are outside b
    for v in vertices:
        error += check_collision_point(v[0], v[1], 0.2,
                                       b_x - w/2.0, b_x + w/2.0,
                                       b_y - h/2.0, b_y + h/2.0, s)
