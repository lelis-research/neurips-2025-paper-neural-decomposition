...
PLOT_NAME = os.environ.get("PLOT_NAME", f"ComboGrid{GAME_WIDTH}*{GAME_WIDTH}")
...
class arguments:
    ...
    # ----- plot setting -----
    pattern                                      = BASELINES
    baselines_spec: dict = field(default_factory=lambda: {
        "Didec": ("red", "--", "Didec"),
        "Vanilla": ("black", "-", "Vanilla"),
        "Fine-Tuned Policy": ("blue", "-o", "Fine-Tuned Policy"),
        "Transferred Policy": ("green", "-.", "Transferred Policy"),
        "DecWhole": ("purple", ":*", "DecWhole"),

        "Didec neurons-only": ("orange", ":", "Didec neurons-only"),
        "Didec input-only": ("brown", "-.", "Didec input-only"),
    })
