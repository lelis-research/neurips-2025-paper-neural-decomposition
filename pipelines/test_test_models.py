...
length_threshold = 10    # goal reached in ~25 steps
    ...
    for directory_path in directory_paths:
        ...
        for model in models:
            ...
            if isinstance(checkpoint, dict):
                ...
                episode_lengths = checkpoint['episode_length']
                entropies = checkpoint['policy_entropy']
                steps = ['steps']
            else:
                ...
                if isinstance(checkpoint, dict):
                    ...
                    (log_data['episode_length'] < length_threshold)
                        ...
                        if first_converged_index is not None:
                            convergence_step = log_data.loc[first_converged_index, 'step']
                        else:
                            convergence_step = float("inf")
                        
                        if best_model_per_seed[seed]['model'] is None or (current_length < best_model_per_seed[seed]['length'] or convergence_step <= best_model_per_seed[seed]['convergence_step']):
        ...
        if len(best_model_per_seed) != 0:
            for best_seed in best_model_per_seed:
                logger.info(f"Best model for seed {best_seed} and env {PROBLEM_NAMES[env_seed] if args.env_id == 'ComboGrid' else env_seed} is {best_model_per_seed[best_seed]['model']} with length {best_model_per_seed[best_seed]['length']} \
                            Convergence rate is {best_model_per_seed[best_seed]['convergence_step'] if 'convergence_step' in best_model_per_seed[best_seed] else "NA"}.")
