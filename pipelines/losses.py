...
class LevinLossActorCritic:
    ...
    def _run(self, env: ComboGym, mask: list, agent: GruAgent, numbers_steps: int):
        ...
        if self.mask_type == "internal":
            ...
            if self.mask_transform_type == "quantize":
                ...
                trajectory = agent.run_with_input_mask_softmax(envs=env, mask_f=mask[0], mask_a=mask[1], max_size_sequence=numbers_steps)
    ...
    def loss(self, feature_masks, actor_masks, models, trajectory, number_actions, joint_problem_name_list, problem_str, number_steps):
        """
    ...
    def compute_loss(self, feature_masks, actor_masks, agents, problem_str, trajectories, number_actions, number_steps):
        """
                ...
                if j < len(t):
                    for i in range(len(options)):
                        ...
                        if self.is_applicable(t, actions, j):
                            ...
                            # else:
                            # mask_name = 'o' + str(i) + "-" + str(options[i].feature_mask)
                            # mask_name += 'o' + str(i) + "-" + str(options[i].actor_mask)
            ...
            for direction in directions:
                ...
                for i in range(game_width):
                    for j in range(game_width):    
                        ...
                        actions = self._run(env, agent.feature_mask, agent.actor_mask, agent, agent.option_size)
